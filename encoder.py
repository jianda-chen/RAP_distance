...
class PixelEncoder(nn.Module):
    ...
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32, stride=None):
        ...
        four_layers_dims = 35
        if obs_shape[-1] == obs_shape[-2]:
            if obs_shape[-1] == 84:
                four_layers_dims = 35
            elif obs_shape[-1] == 128:
                four_layers_dims = 57
            elif obs_shape[-1] == 168:
                four_layers_dims = 77
            else:
                raise NotImplementedError
                
        out_dim = {2: 39, 4: four_layers_dims, 6: 31}[num_layers]
...
class PixelEncodeRobo168(PixelEncoder):
    """Convolutional encoder of pixels observations."""
