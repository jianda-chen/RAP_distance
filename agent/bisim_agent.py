...
class BisimAgent(object):
    ...
    def update_encoder(self, obs, action, reward, L, step):
        ...
        z_dist = F.mse_loss(h, h2, reduction='none')#.mean(dim=-1, keepdim=True)
        ...
        if self.transition_model_type == '':
            ...
            # transition_dist  = F.smooth_l1_loss(pred_next_latent_mu1, pred_next_latent_mu2, reduction='none')#.mean(dim=-1, keepdim=True)
